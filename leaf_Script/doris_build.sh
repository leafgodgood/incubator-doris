...
cp -rf /opt/doris_$time/output/* /opt/doris_$time/doris_1_$time
cp -rf /opt/doris_$time/output/* /opt/doris_$time/doris_2_$time
cp -rf /opt/doris_$time/output/* /opt/doris_$time/doris_3_$time
#time=220319023122
IP=127.0.0.1
first_port=40000
last_port=40100
...
# 容器启动
docker-compose -f doris_install_$time.yml up >> $install_log 2>&1
...
done < ./host_ip_$time.txt      # 读取存储ip的文件
...
#设置环境变量
ansible 172.$ip_subnet.0.2 -m shell -a "echo 'export JAVA_HOME=/usr/lib/jvm/java-11' >> .bashrc"
ansible 172.$ip_subnet.0.3 -m shell -a "echo 'export JAVA_HOME=/usr/lib/jvm/java-11' >> .bashrc"
ansible 172.$ip_subnet.0.4 -m shell -a "echo 'export JAVA_HOME=/usr/lib/jvm/java-11' >> .bashrc"
#修改配置文件
ansible 172.$ip_subnet.0.2 -m shell -a "sed 's/4096/2048/g' /opt/doris_$time/doris_1_$time/fe/conf/fe.conf"
ansible 172.$ip_subnet.0.3 -m shell -a "sed 's/4096/2048/g' /opt/doris_$time/doris_2_$time/fe/conf/fe.conf"
ansible 172.$ip_subnet.0.4 -m shell -a "sed 's/4096/2048/g' /opt/doris_$time/doris_3_$time/fe/conf/fe.conf"
#启动fe_master
ansible 172.$ip_subnet.0.2 -m shell "bash /opt/doris_$time/doris_1_$time/fe/bin/start_fe.sh --daemon"
#判断fe_master是否启动成功
ansible 172.$ip_subnet.0.2 -m shell -a "curl -s 'http://127.0.0.1:8030/api/bootstrap'"
#组建Doris集群
mysql -h172.$ip_subnet.0.2 -P9030 -e "ALTER SYSTEM ADD FOLLOWER '172.$ip_subnet.0.3:9010';"
mysql -h172.$ip_subnet.0.2 -P9030 -e "ALTER SYSTEM ADD FOLLOWER '172.$ip_subnet.0.4:9010';"
mysql -h172.$ip_subnet.0.2 -P9030 -e "ALTER SYSTEM ADD BACKEND '172.$ip_subnet.0.2:9050';"
mysql -h172.$ip_subnet.0.2 -P9030 -e "ALTER SYSTEM ADD BACKEND '172.$ip_subnet.0.3:9050';"
mysql -h172.$ip_subnet.0.2 -P9030 -e "ALTER SYSTEM ADD BACKEND '172.$ip_subnet.0.4:9050';"
#启动fe_follow
ansible 172.$ip_subnet.0.3 -m shell -a "bash /opt/doris_$time/doris_2_$time/fe/bin/start_fe.sh --helper 172.$ip_subnet.0.2:9010 --daemon"
ansible 172.$ip_subnet.0.4 -m shell -a "bash /opt/doris_$time/doris_3_$time/fe/bin/start_fe.sh --helper 172.$ip_subnet.0.2:9010 --daemon"
#启动be
ansible 172.$ip_subnet.0.2 -m shell -a "bash /opt/doris_$time/doris_1_$time/be/bin/start_be.sh --daemon"
ansible 172.$ip_subnet.0.3 -m shell -a "bash /opt/doris_$time/doris_2_$time/be/bin/start_be.sh --daemon"
